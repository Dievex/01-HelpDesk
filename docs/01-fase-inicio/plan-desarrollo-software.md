...
| Elaboración | Fijar una arquitectura que aguante los casos de uso y riesgos más significativos | Casos de uso detallados (los significativos), Modelo de Análisis/Diseño, decisión de stack tecnológico, arquitectura baseline | Lifecycle Architecture Milestone (LCAM) — **cerrado con el [Modelo de Análisis/Diseño](../02-fase-elaboracion/modelo-analisis-diseno/README.md)** |
...
**Cerrada.** UC-20 quedó detallado con las tres decisiones de diseño resueltas (primera respuesta derivada del primer `Comentario` de un Agente, ámbito acotado al Equipo del Supervisor, filtro por rango de `fechaCreacion`). **R-07 se cerró del todo** en la [Lista de Riesgos](lista-riesgos.md). `UC-02 Cerrar Sesión`, que nunca estuvo en el alcance de ninguna iteración de Elaboración, también quedó detallado a continuación. **El catálogo de 43 casos de uso queda completo.** El [Modelo de Análisis/Diseño](../02-fase-elaboracion/modelo-analisis-diseno/README.md) también se completó (7 patrones de flujo cubren los 43 casos de uso), cerrando así los tres artefactos que exige el hito. **Con esto, la Fase de Elaboración queda cerrada (hito LCAM).**
