...
**Cerrada.** UC-20 quedó detallado con las tres decisiones de diseño resueltas (primera respuesta derivada del primer `Comentario` de un Agente, ámbito acotado al Equipo del Supervisor, filtro por rango de `fechaCreacion`). **R-07 se cerró del todo** en la [Lista de Riesgos](lista-riesgos.md). Del catálogo de 43 casos de uso solo queda pendiente `UC-02 Cerrar Sesión`, que nunca estuvo en el alcance de ninguna iteración de Elaboración. El [Modelo de Análisis/Diseño](../02-fase-elaboracion/README.md) sigue sin iniciarse y es, junto con UC-02, lo único que falta para plantearse el cierre de la fase (hito LCAM).
