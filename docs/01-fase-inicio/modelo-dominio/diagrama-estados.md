...
- **No todas las transiciones de este diagrama tienen un caso de uso propio en el catálogo.** `Asignado → EnProgreso` y `Reabierto → EnProgreso` ("Agente inicia/retoma la atención") son ambas de bajo significado: el Agente simplemente empieza a trabajar un ticket que ya es suyo, sin una decisión ni una regla de negocio nueva detrás — a diferencia de `Tomar Ticket` ([UC-11](../../02-fase-elaboracion/especificacion-casos-uso/tickets/UC-11-tomar-ticket.md)), que sí cambia quién tiene el ticket. No se crean casos de uso para transiciones que no lo ameritan solo por completar la lista.
- **`Resuelto → Cerrado` es la única transición que puede disparar el propio Sistema, no un actor.** Existe para que un ticket no se quede en `Resuelto` para siempre si el Solicitante nunca responde — se cierra solo al vencer el **Plazo de Reapertura** (ver [Glosario](../glosario.md)), respaldado por `Ticket.fechaLimiteReapertura` en el [Diagrama de Clases](diagrama-clases.md). El plazo exacto lo define la Organización adoptante; aquí solo se modela que existe y que dispara la misma transición que una confirmación explícita, sin distinguir entre ambos caminos como estados separados — el resultado (`Cerrado`) es el mismo independientemente de por qué se llegó ahí.
