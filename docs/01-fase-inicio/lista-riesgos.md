...
|---|---|---|---|
| R-07 | Complejidad del modelo de dominio (`Equipo`/escalamiento, visibilidad de `ArticuloConocimiento`, `SLA`) subestimada para el Modelo de Diseño | 2026-08-20 | Resuelto del todo: escalamiento y visibilidad ya se habían validado al detallar [UC-13](../02-fase-elaboracion/especificacion-casos-uso/tickets/UC-13-escalar-ticket.md) y [UC-09/UC-10](../02-fase-elaboracion/especificacion-casos-uso/base-conocimiento/UC-09-ver-articulo-conocimiento.md). El último punto pendiente, el cálculo de cumplimiento de SLA, se resolvió al detallar [UC-20 Ver Dashboard de Métricas](../02-fase-elaboracion/especificacion-casos-uso/reportes/UC-20-ver-dashboard-metricas.md): se calcula en vivo (no se almacena), comparando `fechaResolucion` y un `tiempoPrimeraRespuesta` derivado del primer `Comentario` de un Agente contra el `SLA` de la `Prioridad` confirmada del ticket |
